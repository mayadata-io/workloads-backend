...
mysqlQuery.query(createMaya, function (err, results, fields) {
    if (err) {
        ...
        console.log("table is create in percona-jiva")
    }
});

// save 100 person details
router.post('/save', (req, res) => {
    var sql = "INSERT INTO maya.person (rNumber, name, email, age) VALUES ?";
    var values = [];
    ...
    mysqlQuery.query(sql, [values], function (error, results, fields) {
        if (error) {
            console.log('percona-jiva insert error: ' +error  );
            res.status(500).json({ status: 500, message: "Data is not saved" });
        }
        else{
            console.log('percona-jiva The solution is: ', results);
...
router.get('/read/:id', (req, res) => {
    mysqlQuery.query('SELECT * FROM maya.person where rNumber =' + req.params.id, function (error, results, fields) {
        if (error) {
            console.log('percona-jiva insert error: ' +error  );
