...
router.get('/', (req, resp) => {
    ...
    const appnamespace = req.query.appnamespace;
    const targetnamespace = req.query.targetnamespace;
    const appname = req.query.app;
    const volumename = req.query.volumename;
...
kc.loadFromCluster();
const k8sApi = new k8s.Core_v1Api(kc.getCurrentCluster().server);
...
router.get('/litmusstatus', (req, resp) => {
    ...
    k8sApi.listNamespacedPod('litmus').then(res => new Promise(((resolve, reject) => {
...
router.get('/jobstatus', (req, resp) => {
    ...
    const jobname = req.query.jobname;
    console.log(jobname);
    k8sApi.readNamespacedJobStatus(jobname, 'litmus').then(res => new Promise(((resolve, reject) => {
        console.log(res);
        resolve(allStatus);
    })).then((allStatus) => {
        resp.status(200).json(allStatus);
